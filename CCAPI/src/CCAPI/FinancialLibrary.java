...
public class FinancialLibrary {
    ...
    public double SAR(double af, double max, Vector candles, int skipdays) {
        ...
        Candle cref1 = (Candle) candles.elementAt(
                candles.size() - 1 - skipdays - 1);
        ...
        while (!extremefound) {
            cref0 = (Candle) candles.elementAt(
                    candles.size() - 1 - skipdays - index);
            cref1 = (Candle) candles.elementAt(
                    candles.size() - 1 - skipdays - index - 1);
			
            if (uptrend) {
                if (cref1.close > cref0.close) {} else {
                    extremefound = true;
                    break;
                }
            } else {
    ...
    public boolean SARd(double af, double max, Vector candles, int skipdays) {
        ...
        for (int i = 0; i < candles.size() - skipdays; i++) {
            Candle c = (Candle) candles.elementAt(
                    candles.size() - i - 1 - skipdays);
    ...
    public float[] fft(Vector candles, int skipdays) {
        ...
        for (int i = 0; i < out.length; i++) {	
            System.out.println(
                    (out[i] * 1000) + "     Periodenlaenge:" + (i * 2) + "/" + i);
        }
