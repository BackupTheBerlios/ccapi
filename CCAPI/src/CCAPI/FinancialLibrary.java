...
/**
 * 
 * A financial library implementation, use at your own risk - i hope(!) all
 * implementations are correct, please report all errors and bugfixes if possible.
 * 
 * @author us
 */
...
public class FinancialLibrary {
	 ...
	 * @return
	 */
	public double smoothedSlope(int n, int smoothingunits, Vector candles, int skipdays){
			double value=0.0;
	...
	public double SAR(double af, double max, Vector candles, int skipdays) {
		...
		//check first trend
		Candle cref0=(Candle)candles.elementAt(candles.size()-1-skipdays);
		Candle cref1=(Candle)candles.elementAt(candles.size()-1-skipdays-1);
		
		
		if(cref1.close>cref0.close)uptrend=true;
		else uptrend=false;
		...
		while(!extremefound){
			cref0=(Candle)candles.elementAt(candles.size()-1-skipdays-index);
			cref1=(Candle)candles.elementAt(candles.size()-1-skipdays-index-1);
	...
	public boolean SARd(double af, double max, Vector candles, int skipdays) {
		...
		for (int i = 0; i < candles.size() - skipdays; i++) {
			Candle c =
				(Candle) candles.elementAt(candles.size() - i - 1 - skipdays);
			if (hi == 0) {
				hi = c.hi;
				low = c.low;
			} else {
	...
	public double[] dmi(int n, Vector candles, int skipdays) {
		...
		for(int i=0;i<(n);i++){
			...
			if(c0.hi>c1.hi)dm_p+=c0.hi-c1.hi;
			else dm_p+=0;
			
			if(c0.low<c1.low)dm_m+=c0.low-c1.low;
			else dm_m+=0;
			/*
			if(dm_m<dm_p)dm_m+=0;
			if(dm_p<dm_m)dm_p+=0;
			...
			*/
			...
			if(v1>v2 & v1>v3) tr_=v1;
			if(v2>v1 & v2>v3) tr_=v2;
			if(v3>v1 & v3>v2) tr_=v3;
	 ...
	 * @return
	 */
	public double[] bollinger(int n, int deviations,  Vector candles, int skipdays) {
		double[] value = new double[3];
	...
	public float[] fft(Vector candles, int skipdays){
		...
		for(int i=0;i<out.length;i++){	
			System.out.println((out[i]*1000)+ "     Periodenlaenge:"+(i*2)+"/"+i);
		}
