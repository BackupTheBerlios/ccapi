...
public class ConsorsConnection extends Thread {
    ...
    public void initializeConnection() {
        try {
            ...
            url = new URL("https://brokerage-a3.cortalconsors.de/activetrader3/");
    ...
    public void login(String accountno, String pin) {
        ...
        String text = "<?xml version='1.0'?><request servicename='ComplexLogin' language='DE'>\n"
                + " <COMPANY>GE</COMPANY>\n<ACCOUNTNO>" + accountno
                + "</ACCOUNTNO>\n<PIN>" + pin
    ...
    public Quote getOTCQuote(String isin) {
        ...
        if (marketOpen()) {
            ...
            String request = "<?xml version='1.0'?><request servicename='MWGetQuote' language='DE'>"
            ...
            if (root.getAttribute("errorcode").equals("0")) {
                ...
                while (tag != null) {
                    ...
                    if (tag.getNodeName().equals("MAX_BUY_AMOUNT")) {
                        q1.bidvolume = Double.parseDouble(
                                tag.getFirstChild().getNodeValue());
                    } else if (tag.getNodeName().equals("MAX_SELL_AMOUNT")) {
                        q1.askvolume = Double.parseDouble(
                                tag.getFirstChild().getNodeValue());
                    } else if (tag.getNodeName().equals("BUY_QUOTATION")) {
                        q1.ask = Double.parseDouble(
                                tag.getFirstChild().getNodeValue());
                    } else if (tag.getNodeName().equals("SELL_QUOTATION")) {
                        q1.bid = Double.parseDouble(
                                tag.getFirstChild().getNodeValue());
                    } // else
    ...
    public Quote getPublicExchangeQuote(String isin, String exchange) {
        ...
        if (marketOpen()) {
            ...
            if (root.getAttribute("errorcode").equals("0")) {
                ...
                org.w3c.dom.Element rowset = (org.w3c.dom.Element) root.getFirstChild().getNextSibling();

                NodeList rowlist = rowset.getElementsByTagName("ROW");

                for (int i = 0; i < rowlist.getLength(); i++) {
                    org.w3c.dom.Element row = (org.w3c.dom.Element) rowlist.item(
                            i);

                    if (row.getNodeName().equals("ROW")) {
                        // ok. row found.
                        q1 = new Quote();
                        org.w3c.dom.Element e1 = (org.w3c.dom.Element) row.getFirstChild().getNextSibling();

                        while (e1 != null) {
                            if (e1.getNodeName().equals("BID")) {
                                q1.bid = Double.parseDouble(
                                        e1.getFirstChild().getNodeValue());
                                ...
                                q1.bidvolume = Double.parseDouble(
                                    ...
                                    e1.getNodeName().equals("STOCKEXCHANGE_ID")) {
                                q1.exchange = (e1.getFirstChild().getNodeValue());
                            } else if (
                                    e1.getNodeName().equals("STOCKEXCHANGE_NAME")) {
                                q1.exchange_name = (e1.getFirstChild().getNodeValue());
                            } else if (e1.getNodeName().equals("SECURITY_NAME")) {
                                q1.security_name = (e1.getFirstChild().getNodeValue());
                                System.out.println(
                                        "Security name:" + q1.security_name);
                                ...
                                q1.quotation = Double.parseDouble(
                                        e1.getFirstChild().getNodeValue());
                            }

                            e1 = (org.w3c.dom.Element) e1.getNextSibling().getNextSibling();
                        }
    ...
    public Document sendRequest(String request) {
        ...
        saveToLog(
                "--- REQ:    " + (new java.util.Date()).toGMTString() + "\n"
                + request);
        ...
        try {
            ...
            saveToLog(
                    "--- RES:     " + (new java.util.Date()).toGMTString()
                    + "\n" + ret);
