...
public class ConsorsConnection extends Thread {
	...
	public void login(String accountno, String pin) {
		...
		if(debug)System.out.println("RESP:" + response.toString());
		...
		if (marketOpen()) {
			...
			String request =
				"<?xml version='1.0'?><request servicename='MWGetQuote' language='DE'>"
					+ "<ORDERTYPE>B</ORDERTYPE><SECURITY_WKN>"
					+ isin
					+ "</SECURITY_WKN>"
					+ "<SECURITY_NOMINAL_AMOUNT>1000.0</SECURITY_NOMINAL_AMOUNT>"
					+ "<STOCKEXCHANGE_ID>OTC</STOCKEXCHANGE_ID>"
					+ "</request>";
			...
			if (root.getAttribute("errorcode").equals("0")) {
				...
				if(debug)System.out.println("" + root.getFirstChild().getClass());

				if(debug)System.out.println("" + root.getFirstChild().getNextSibling().getClass());

				org.w3c.dom.Element tag =
					(org.w3c.dom.Element) root.getFirstChild();
				while (tag != null) {
					if(debug)System.out.println("parsing tag:" + tag.getNodeName());
	...
	public Quote getPublicExchangeQuote(String isin, String exchange) {
		...
		if (marketOpen()) {
			...
			if (root.getAttribute("errorcode").equals("0")) {
				...
				org.w3c.dom.Element rowset =
					(org.w3c.dom.Element) root.getFirstChild().getNextSibling();

				NodeList rowlist = rowset.getElementsByTagName("ROW");
				for (int i = 0; i < rowlist.getLength(); i++) {
					org.w3c.dom.Element row =
						(org.w3c.dom.Element) rowlist.item(i);
	...
	public Document sendRequest(String request) {
		...
		if(debug)System.out.println("REQ: "+request);
	...
	public void initializeLogger() {
		try {
			String sessionstart =
				(new java.util.Date().toGMTString()).replaceAll(" ", "_");
