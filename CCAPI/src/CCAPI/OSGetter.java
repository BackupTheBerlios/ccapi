...
/*
 * Ulrich Staudinger Created on 03.07.2004
 *  
 */
...
public class OSGetter {
	...
	private void get(String wkn) {
		...
		String preurl =
			"http://www.euwax.de/finder/mta_finder/fnd_build_mta.php?wp_keyword=&fnd_typ=alle&region_basiswert=alle&basiswert=";
		String posturl =
			"&optionsart=alle&basispreis_ab=&basispreis_bis=&lauf_ab=&lauf_bis=&fndr_sel=os&sort_me=&asc_desc=&nln_min=&nln_max=&special_sel=&back_url=%2Fprod_optionsscheine%2Foptionsscheine.htm&back_target=_self&go.x=21&go.y=8&cutoffhere=&sort_me=&asc_desc=&ll=1&mm=28&page=";
		...
		try {
			...
			++firstindex;
			if (!baseIsin.equals("")) {
				if(debug)System.out.println("Getting from index " + firstindex);
				...
				while (l != null) {
					...
					if (l.indexOf("<! ##### Ende Seitenkopf ###### !>") != -1)
						inTableArea = true;
					l = din.readLine();
...
private void parseTable(String table){
	...
	public Vector getOptions(String wkn, double min, double max){
		...
		for(int i=0;i<options.size();i++){
			...
			if(v1>=min && v1<=max)ret.addElement(o);
		}
		options=ret;
		return ret;
	}
	...
	public static void main(String[] args) {
		...
		if(!(args.length>0)){
			...
			//os.get(args[0]);
			if(args.length==1)os.getOptions(args[0]);
			else os.getOptions(args[0], Double.parseDouble(args[1]), Double.parseDouble(args[2]));
			os.dump();
