...
public class ConsorsQuoteRetriever{
	...
	*/
	static public Vector getHistory(String isin){
		...
		try{
			URL url=new URL("http://chartdata.consors.onvista.de/data/quotes.html");
			HttpURLConnection hpcon =
				(HttpURLConnection) url.openConnection();
			hpcon.setRequestProperty(
				"POST",
				"/data/quotes.html HTTP/1.1");
		...
		//	String request="message=%23+DATAOBJECT%0AQUERY%0A%23+Function%0AgetLongtermHistory%0A%23+UserId%0Auser%0A%23+ISIN%0A99456%0A%23+Exchange%0AGER%0A%23+BeginDate%0A%0A%23+EndDate%0A%0A%23%0A";
			...
			String request="message=%23+DATAOBJECT%0AQUERY%0A%23+Function%0AgetLongtermHistory%0A%23+UserId%0Auser%0A%23+ISIN%0A"+isin+"%0A%23+Exchange%0AGER%0A%23+BeginDate%0A%0A%23+EndDate%0A%0A%23%0A";
...
//			InputStream is=url.openStream();
			DataInputStream din = new DataInputStream(new BufferedInputStream(hpcon.getInputStream()));
			...
			while (l != null) {
				...
				if(l.startsWith("#Datetime;Open;High;Low;Close"))quotes=true;

				if(l.startsWith("# Name"))name=din.readLine();
				if(l.startsWith("# ISIN"))isin2=din.readLine();
...
//				l+= new String(l.getBytes("UTF-8"));
		...
		System.out.println("ConsorsQuoteRetriever Read "+ret.size()+" candles");
	...
	static public String search(String q){
		try{
			URL url=new URL("http://chartdata.consors.onvista.de/data/search.html");
			HttpURLConnection hpcon =
				(HttpURLConnection) url.openConnection();
			hpcon.setRequestProperty(
				"POST",
				"/data/search.html HTTP/1.1");
		...
		//	String request="message=%23+DATAOBJECT%0AQUERY%0A%23+Function%0AgetLongtermHistory%0A%23+UserId%0Auser%0A%23+ISIN%0A99456%0A%23+Exchange%0AGER%0A%23+BeginDate%0A%0A%23+EndDate%0A%0A%23%0A";
			...
			String request="message=%23+DATAOBJECT%0AQUERY%0A%23+Function%0Asearch%0A%23+UserId%0Auser%0A%23+Pattern%0A"+q+"%0A%23%0A";
...
//			InputStream is=url.openStream();
			...
			//read all line wise and append to ret.
			String l = din.readLine();
			if(l==null)System.out.println("LINE = NULL!!!!");	
			while (l != null) {
